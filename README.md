...
Set `KOMETA_CONFIG_PATH` to an independently managed Kometa checkout containing its `config.yml`, assets, metadata, and overlays. `KOMETA_RUNTIME_CONFIG_PATH` defaults to the checkout's `config.yml`; set it to an existing ignored private YAML file when credentials are stored separately. Both Kometa and PATTRMM use that file, and regeneration preserves your selection. Set `IMAGEMAID_PLEX_PATH` to Plex application data containing `Cache`, `Metadata`, and `Plug-in Support`.
