...
#
.PHONY: $(ALL) $(DOWN) $(CLEAN) $(BUILD_DEPENDS) $(PIA_CREDS) $(BUILD) $(UP) $(START) \
	$(TEST_VPN) $(LOGS) $(OPEN) $(HELP) $(RUN) $(PRINT_CONFIG) $(PRINT_ENV $(EVAL_ENV)
...
$(PIA_CREDS):
	@if [ -z "${PIA_USER}" -o -z "${PIA_PASS}" ]; then \
		echo "Please set PIA_USER and PIA_PASS"; \
		exit 1; \
	fi
...
#
# $(EVAL_ENV): Prints the evaluated docker compose default env configuration.
#
$(EVAL_ENV):
...
$(HELP):
	...
	@echo "  $(EVAL_ENV)        - Prints the evaluated docker compose default env configuration."
