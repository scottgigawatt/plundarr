...
Maraudarr documentation should explain contracts and intent without duplicating
the implementation. Type annotations remain the source of truth for types;
docstrings describe meaning, side effects, invariants, and failure behavior.

## Public APIs

Public modules, classes, methods, and functions use Google-style sections when
...
Private helpers are filtered out of the generated API reference, but they are
not undocumented. Give every non-obvious helper a concise docstring that states
its transformation or safety rule. Add inline comments at decision points where
the reason cannot be inferred from the code.
