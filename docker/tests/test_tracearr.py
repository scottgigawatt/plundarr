...
class TracearrTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # Validation tests mutate catalog models, so no test may share another test's instance.
    ...
    def test_named_volumes_follow_selection_without_global_names(self) -> None:
        ...
        # Let Compose scope storage by project so independent deployments cannot share it accidentally.
        self.assertNotIn("name:", footer)
        self.assertNotIn("external:", footer)

        # One catalog choice expands into three Compose services without exposing internal choices.
        ...
        # Keep database and queue updates coordinated and their ports off the host.
    ...
    def test_catalog_rejects_invalid_and_duplicate_volume_declarations(self) -> None:
        ...
        # Restore the valid owner before introducing a conflicting declaration in another service.
    ...
    def test_secrets_and_existing_state_survive_regeneration(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            # Exercise real file generation while leaving external Compose invocation to its own tests.
            with patch("maraudarr.render.validate_compose"):
                ...
                # Secrets belong only in the restricted deployment environment, never its public example.
                ...
                # Remove and re-add monitoring to prove credentials and application state survive both transitions.
                write_stack(
    ...
    def test_homepage_monitor_combinations_and_internal_port(self) -> None:
        ...
        for monitors in (set[str](), {"tracearr"}, {"tautulli"}, {"tracearr", "tautulli"}):
            with self.subTest(monitors=monitors):
                ...
                # Homepage widgets use the internal service port, independently of the browser-facing host port.
