...
Avast, matey! This hold stores the configuration scrolls fer the optional **Sonarr Anime** addon. It runs as a second Sonarr instance with its own database, settings, port, and anime TV root.
...
Sonarr Anime sees download clients at `/downloads` and imports finished anime into `/tv`. In Plundarr, `/tv` maps to `HOST_ANIME_TV_PATH` for this addon only.
...
Render the addon into yer final chart:
